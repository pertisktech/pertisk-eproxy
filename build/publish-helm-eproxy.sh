#!/usr/bin/env bash
# Package pertisk-eproxy Helm chart and upload to the Pertisk chart repository.
#
# Usage:
#   VERSION=0.1.0 ./build/publish-helm-eproxy.sh
#   HELM_CHART_TOKEN=<jwt> VERSION=0.1.0 ./build/publish-helm-eproxy.sh
#
# Auth (one of):
...
# Default image tag follows chart appVersion when empty.
sed -i 's/^  tag: .*/  tag: ""/' "$WORKDIR/chart/values.yaml"

CHART_TGZ="$WORKDIR/pertisk-eproxy-${VERSION}.tgz"
...
if [ ! -f "$CHART_TGZ" ]; then
  echo "helm package did not produce $CHART_TGZ" >&2
...
if [ "$HTTP_CODE" -ge 200 ] && [ "$HTTP_CODE" -lt 300 ]; then
  ...
  echo ""
  echo "Published pertisk-eproxy ${VERSION} (image: ${HARBOR_INGRESS_IMAGE}:v${VERSION})"
  echo "Chart: ${HELM_CHART_REPO_URL%/}/charts/pertisk-eproxy"
  echo "Install: helm repo add pertisk ${HELM_CHART_REPO_URL%/} && helm upgrade --install pertisk-eproxy pertisk/pertisk-eproxy --version ${VERSION} -n pertisk-eproxy --create-namespace"
  exit 0
fi
