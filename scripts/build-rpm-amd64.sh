...
if [[ -z "$ERLEXEC_DESC" ]] || [[ "$ERLEXEC_DESC" != *"x86-64"* ]]; then
  echo "Release architecture mismatch for x86_64 package." >&2
...
if [[ -n "$CRYPTO_SO_PATH" ]]; then
  ...
  if [[ "$TARGET_OPENSSL_ABI" != "any" ]] && [[ "$RELEASE_OPENSSL_ABI" != "unknown" ]] && [[ "$TARGET_OPENSSL_ABI" != "$RELEASE_OPENSSL_ABI" ]]; then
    ...
    echo "Example (OpenSSL 1.1 target): RELEASE_BUILD_IMAGE=erlang:27-bullseye TARGET_OPENSSL_ABI=1 make package-rpm-amd64" >&2
    exit 1
  fi
fi
