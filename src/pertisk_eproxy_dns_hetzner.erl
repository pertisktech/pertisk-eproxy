%% @doc Hetzner DNS API v1 for ACME DNS-01 TXT records.
-module(pertisk_eproxy_dns_hetzner).
...
-define(API, <<"https://dns.hetzner.com/api/v1">>).

-spec resolve_zone(binary(), binary() | undefined, binary()) -> {ok, map()} | {error, term()}.
resolve_zone(ApiToken, Zone0, _Host) when is_binary(Zone0), byte_size(Zone0) > 0 ->
    Zone = string:lowercase(trim_space_binary(Zone0)),
    case get_zone_by_name(ApiToken, Zone) of
        {ok, ZoneMap} -> {ok, ZoneMap};
...
try_zones(Token, [Zone | Rest], FirstFatalErr) ->
    case get_zone_by_name(Token, Zone) of
        {ok, ZoneMap} -> {ok, ZoneMap};
        {error, Err} ->
...
get_zone_by_name(ApiToken, ZoneName) ->
    Url = <<?API/binary, "/zones?name=", ZoneName/binary>>,
    case http_get(ApiToken, Url) of
        {ok, #{<<"zones">> := [Z | _]}} when is_map(Z) ->
            case {maps:get(<<"id">>, Z, undefined), maps:get(<<"name">>, Z, undefined)} of
                {Id, Name} when Id =/= undefined, Name =/= undefined ->
                    {ok, #{zone_id => Id, zone_name => lower_bin(Name)}};
                _ ->
                    {error, {zone_lookup, Z}}
            end;
        {ok, #{<<"zones">> := []}} ->
...
is_zone_not_found_error(zone_not_found) -> true;
...
create_txt(ApiToken, ZoneId, RecordName, TxtContent) ->
    Url = <<?API/binary, "/records">>,
    Body = #{
        <<"zone_id">> => ZoneId,
        <<"type">> => <<"TXT">>,
        <<"name">> => RecordName,
    ...
    case http_post_json(ApiToken, Url, Body) of
        ...
        {ok, Other} ->
            {error, {missing_record, Other}};
        {error, _} = E ->
            E
    end.

-spec delete_txt(binary(), binary(), term()) -> ok | {error, term()}.
delete_txt(ApiToken, _ZoneId, RecordId) ->
    Rid = id_to_binary(RecordId),
    Url = <<?API/binary, "/records/", Rid/binary>>,
...
http_get(Token, Url) ->
    ...
    case httpc:request(get, Req, http_opts(), []) of
        {ok, {{_, 200, _}, _RespH, RespB}} -> decode_json(RespB);
...
http_post_json(Token, Url, BodyMap) ->
    ...
    case httpc:request(post, Req, http_opts(), []) of
        {ok, {{_, 200, _}, _RespH, RespB}} -> decode_json(RespB);
        {ok, {{_, 201, _}, _RespH, RespB}} -> decode_json(RespB);
        {ok, {{_, Status, _}, _RespH, RespB}} -> {error, {http, Status, RespB}};
        {error, R} -> {error, R}
    end.
