%% @doc deSEC DNS API v1 for ACME DNS-01 TXT records.
-module(pertisk_eproxy_dns_desec).
...
-define(API, <<"https://desec.io/api/v1">>).
...
create_txt(ApiToken, Domain, SubName, TxtContent) ->
    Url = <<?API/binary, "/domains/", Domain/binary, "/rrsets/">>,
    ...
    case http_patch_json(ApiToken, Url, Body) of
        {ok, _} -> {ok, {desec, ApiToken, Domain, SubName}};
        {error, _} = E -> E
    end.

-spec delete_txt(binary(), binary(), binary()) -> ok | {error, term()}.
delete_txt(ApiToken, Domain, SubName) ->
    Url = <<?API/binary, "/domains/", Domain/binary, "/rrsets/">>,
    Body = [#{
        <<"subname">> => SubName,
        <<"type">> => <<"TXT">>,
...
http_patch_json(Token, Url, BodyTerm) ->
    Enc = thoas:encode(BodyTerm),
